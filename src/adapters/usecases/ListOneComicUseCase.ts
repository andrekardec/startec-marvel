...
class ListOneComicUseCase implements IListOneComicUseCase{
  ...
  async execute(id: number): Promise<IComicDTO | undefined> {
      ...
      if(response.status !== 404) {
        const creatorsData = response.data.data.results[0].creators.items.map(items => items);

        const creators = creatorsData.slice(0,2).map((({ name, role}) => ({ name, role })));
  
        this.comics = response.data.data.results.map((({id, title, modified, resourceURI, thumbnail, images, prices, series, stories}) => ({
